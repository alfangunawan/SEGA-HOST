...
@section('content')
    <div class="space-y-6">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div
                class="rounded-xl border border-indigo-100 bg-indigo-50 p-5 dark:bg-indigo-500/10 dark:border-indigo-500/30">
                <p class="text-sm font-medium text-indigo-700 dark:text-indigo-200">{{ __('Total Peminjaman Selesai') }}</p>
                <p class="mt-2 text-2xl font-semibold text-indigo-900 dark:text-indigo-100">
                    {{ number_format($summary['count']) }}</p>
            </div>
            <div
                class="rounded-xl border border-emerald-100 bg-emerald-50 p-5 dark:bg-emerald-500/10 dark:border-emerald-500/30">
                <p class="text-sm font-medium text-emerald-700 dark:text-emerald-200">{{ __('Total Pendapatan') }}</p>
                <p class="mt-2 text-2xl font-semibold text-emerald-900 dark:text-emerald-100">Rp
                    {{ number_format($summary['revenue'], 2, ',', '.') }}</p>
            </div>
        </div>
            ...
            <form action="{{ route('admin.rekap.index') }}" method="GET" class="flex flex-wrap items-center gap-3">
                <div class="relative">
                    <input type="text" name="search" placeholder="{{ __('Cari nama penyewa atau unit...') }}"
                        value="{{ $search }}"
                        class="w-64 rounded-md border-gray-300 pl-10 pr-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100">
                ...
                <input type="text" name="date_range" placeholder="{{ __('YYYY-MM-DD to YYYY-MM-DD') }}"
                    value="{{ $dateRange }}"
                    class="w-56 rounded-md border-gray-300 px-3 py-2 text-sm shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100"
                    aria-describedby="date_range_help">
                <span id="date_range_help"
                    class="sr-only">{{ __('Gunakan format tanggal: 2025-01-01 to 2025-01-31') }}</span>
                <button type="submit"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-300">
                                {{ __('Belum ada peminjaman yang selesai.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
