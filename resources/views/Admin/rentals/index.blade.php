        ...
        <div class="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm dark:bg-slate-900 dark:border-slate-800 dark:shadow-none">
            <table class="min-w-full divide-y divide-gray-200 dark:divide-slate-800">
                <thead class="bg-gray-50 dark:bg-slate-900/60">
                    <tr>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">{{ __('Penyewa') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">{{ __('Unit') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">{{ __('Periode') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">{{ __('Status') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">{{ __('Denda') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider dark:text-gray-300">{{ __('Total Biaya') }}</th>
                        <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider w-32 dark:text-gray-300">{{ __('Aksi') }}</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-slate-900 dark:divide-slate-800">
                    @forelse ($rentals as $rental)
                        <tr>
                            <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-100">{{ $rental->user->name ?? '-' }}</td>
                            ...
                            <td class="px-4 py-3 text-sm">
                                <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold
                                    @class([
                                        'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-200' => $rental->status === 'ongoing',
                                        'bg-sky-100 text-sky-700 dark:bg-sky-500/20 dark:text-sky-200' => $rental->status === 'returned',
                                        'bg-rose-100 text-rose-700 dark:bg-rose-500/20 dark:text-rose-200' => $rental->status === 'late',
                                    ])">
                                    {{ match ($rental->status) {
                                        'ongoing' => __('Sedang Berjalan'),
                                        'returned' => __('Dikembalikan'),
                            ...
                            <td class="px-4 py-3 text-sm text-gray-700 dark:text-gray-100">Rp {{ number_format($rental->total_cost, 2, ',', '.') }}</td>
                            <td class="px-4 py-3 text-sm text-gray-500 dark:text-gray-300">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.rentals.edit', $rental) }}" class="text-indigo-600 hover:text-indigo-500 font-medium dark:text-indigo-300 dark:hover:text-indigo-200">{{ __('Edit') }}</a>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-300">{{ __('Belum ada data peminjaman.') }}</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
