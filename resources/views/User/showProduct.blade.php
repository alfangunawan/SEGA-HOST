            ...
            @if($units->count() > 0)
                <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 mb-12">
                    @foreach($units as $unit)
                        <div class="group relative bg-white dark:bg-gray-800 rounded-3xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden hover:shadow-2xl hover:scale-[1.02] transition-all duration-300">
                            <!-- Server Visual -->
                            <div class="relative h-56 bg-blue-50 dark:bg-gray-700 flex items-center justify-center overflow-hidden">
                                <div class="absolute inset-0 bg-blue-100 dark:bg-gray-600 group-hover:bg-blue-200 dark:group-hover:bg-gray-500 transition-all duration-300"></div>
                                <div class="relative text-center z-10">
                                    <div class="w-20 h-20 mx-auto mb-4 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg group-hover:scale-110 transition-transform duration-300">
                                        <svg class="w-10 h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 12h14M5 12a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v4a2 2 0 01-2 2M5 12a2 2 0 00-2 2v4a2 2 0 002 2h14a2 2 0 002-2v-4a2 2 0 00-2-2"></path>
                                        </svg>
                                    </div>
                                    <span class="inline-block px-3 py-1 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-300 text-sm font-medium rounded-full shadow-sm">
                                        {{ $unit->code }}
                                    </span>
                                </div>
                                <!-- Status Badge -->
                                <div class="absolute top-4 right-4">
                                    <span class="px-3 py-1 bg-emerald-500 text-white text-xs font-semibold rounded-full shadow-lg">
                                        {{ ucfirst($unit->status) }}
                                    </span>
                                </div>
                            </div>
                                ...
                                @if($unit->categories->count() > 0)
                                    <div class="flex flex-wrap gap-2 mb-4">
                                        @foreach($unit->categories as $category)
                                            <span class="px-3 py-1 bg-blue-50 dark:bg-blue-900 text-blue-600 dark:text-blue-400 text-xs font-medium rounded-full border border-blue-200 dark:border-blue-800">
                                                {{ $category->name }}
                                            </span>
                                        @endforeach
                                    </div>
            ...
            @else
                ...
                <div class="text-center py-20">
                    <div class="max-w-md mx-auto">
                        <div class="w-24 h-24 mx-auto mb-6 bg-gray-100 dark:bg-gray-700 rounded-3xl flex items-center justify-center">
