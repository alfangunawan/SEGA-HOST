    ...
    public function store(Request $request): RedirectResponse
    {
        try {
            ...
            $validated = $request->validate([
                ...
                'rental_days' => ['required', 'integer', 'min:1', 'max:5'], // Max 5 hari untuk prepaid
            ...
            // Prepaid amount: bayar untuk maksimal 5 hari atau sesuai rental_days
            $prepaidDays = min($rentalDays, 5);
            $prepaidAmount = $unit->price_per_day * $prepaidDays;

            $rental = Rental::create([
                'user_id' => $user->id,
                'unit_id' => $unit->id,
                'start_date' => $startDate,
                'end_date' => $startDate->copy()->addDays($rentalDays),
                'status' => 'pending',
                'total_cost' => $unit->price_per_day * $rentalDays,
            ...
            return redirect()->route('rentals.payment', $rental)
                ->with('success', 'Please complete payment to confirm your rental.');
